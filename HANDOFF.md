...
- **Engine export adapter (Godot 4)** — the export is vertical-neutral by default; a vertical can declare an `engine` (the per-vertical export-adapter hook on `verticals::Vertical`). `game_2d` → `Engine::Godot`. `POST /export` takes an optional `target` (`"godot"`); when it matches the project's vertical the pack becomes a **drop-in Godot project**: the same textures + a sibling `<file>.import` per texture (2D-sprite import settings; the machine-local cache/uid are intentionally omitted so Godot regenerates them on first import while keeping our `[params]`) + a minimal `project.godot` + a README. Unknown target → 400; target a vertical doesn't support → 400. The packer is `backend/src/export/godot.rs`; the Export dialog shows a Generic/Godot toggle when the vertical supports an engine. Unity is the next target behind the same hook.

## Code map
- `backend/src/routes/` — `auth, workspaces, projects, canon, assets, audio, collections, comments, lineage, export, search, context, recipes, activity` · vertical registry in `src/verticals.rs` (incl. the per-vertical `engine` export hook) · engine packers in `src/export/` (`godot.rs`).
