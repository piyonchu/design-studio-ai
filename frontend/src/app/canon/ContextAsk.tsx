...
export function ContextAsk({ projectId }: { projectId: string }) {
  ...
  const [hits, setHits] = useState<api.ContextHit[] | null>(null)
  ...
  async function ask(e: FormEvent) {
    ...
    setNote(null)
    try {
      setHits(await api.askContext(projectId, query))
    } catch (err) {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
        {hits == null ? (
          <p className="px-2 py-6 text-center text-xs text-text-dim">
            Ask a question to retrieve the most relevant project context.
          </p>
        ) : hits.length === 0 ? (
          <p className="px-2 py-6 text-center text-xs text-text-dim">
